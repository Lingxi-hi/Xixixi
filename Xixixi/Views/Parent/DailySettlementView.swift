...
struct DailySettlementView: View {
        ...
        VStack(spacing: 16) {
            ...
            HStack(spacing: 0) {
                summaryCell(value: "\(store.approvedTasksToday.count)/\(store.dailyTasks.count)",
                            label: "完成任务", symbol: "checkmark.circle.fill", color: Color.xSecondary)
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                GeometryReader { geo in
                    ZStack(alignment: .leading) {
                        RoundedRectangle(cornerRadius: 4).fill(Color.xSubtext.opacity(0.15))
                        RoundedRectangle(cornerRadius: 4).fill(Color.xSecondary)
    ...
    private func summaryCell(value: String, label: String, symbol: String, color: Color) -> some View {
        VStack(spacing: 4) {
        ...
        HStack(spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                Text(store.pet.name)
                    ...
                    .foregroundColor(Color.xText)
                HStack(spacing: 5) {
                    Image(systemName: store.pet.mood.sfSymbol)
                        .font(.system(size: 13))
                        .foregroundStyle(store.pet.mood.color)
                    Text(store.pet.mood.description)
                        .font(.system(size: 14, weight: .medium, design: .rounded))
                        .foregroundColor(store.pet.mood.color)
                }
                HStack(spacing: 10) {
                    miniBar("fork.knife", value: store.pet.hunger,    color: .orange)
                    miniBar("drop.fill",  value: store.pet.thirst,    color: .blue)
                    miniBar("heart.fill", value: store.pet.moodScore, color: .pink)
                }
            }
        }
        .padding(16)
    ...
    private func miniBar(_ symbol: String, value: Int, color: Color) -> some View {
        HStack(spacing: 3) {
